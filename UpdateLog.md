...
- 🛠️[修复]-统一运行时与 Source Generator 的文化元数据校验和名称规范化，并正确识别 RESX 的中性文化文件名。
- 🧪[新增]-增加 JSON/XML 非法文化元数据测试。
...
- 🛠️[修复]-修正发布脚本的默认 Demo 路径和发布参数校验，并将版本更新脚本改为维护根目录四段 Version。

## 12.1.2.9 (2026-09-22)

- 📝[维护]-同步运行时目标框架、文化回退规则、依赖版本审计文档，并补齐 `Lang.Avalonia.Analysis` 的 NuGet 包元数据。

## 12.1.2.8 (2026-09-22)

- 📝[优化]-补充 JSON/XML 资源文件和嵌入资源的元数据诊断，避免无效文件静默跳过。

## 12.1.2.7 (2026-09-22)

- 🛠️[修复]-增强 Source Generator 的 Key 合并、C# 标识符清洗、字面量转义和输入诊断。
- 🧪[新增]-增加 Source Generator 边界测试。

## 12.1.2.6 (2026-09-22)

- 🐛[修复]-固定文化的格式化参数现在使用选定文化，而不是当前绑定文化。

## 12.1.2.5 (2026-09-22)

- 🛡️[优化]-为管理器和资源插件增加同步边界，并让语言列表和加载诊断返回独立快照。

## 12.1.2.4 (2026-09-22)

- 🐛[修复]-修复嵌入资源在注册前追加时被清除的问题，并让动态追加资源刷新现有绑定。

## 12.1.2.3 (2026-09-22)

- 🐛[修复]-统一 JSON、XML、RESX 的文化名称规范化、父文化回退和空字符串资源行为。
- 🧪[新增]-增加资源文化查找自动化测试。
