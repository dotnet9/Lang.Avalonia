...
3. `GetResource(key, cultureName)` 必须先使用显式文化或当前文化，并按父文化逐级回退；仍未命中时检查默认文化及其父文化，最后返回原始 Key。
...
6. 插件按显式文化或当前文化、对应父文化、默认文化及其父文化、原始 Key 的顺序回退，并避免重复检查相同文化。
...
2. 第一步文化的父文化链。
3. 初始化时传入的默认文化及其父文化链；已经检查过的文化不会重复检查。
4. 原始 Key。
...
`cultureName` 必须是有效的 .NET `CultureInfo` 名称；插件和 Source Generator 会统一规范化文化名称，并拒绝缺失、空白或非法文化元数据。
...
根节点的 `cultureName` 必须是有效的 .NET `CultureInfo` 名称；非法元数据不会进入运行时资源缓存或 Source Generator 输出。
